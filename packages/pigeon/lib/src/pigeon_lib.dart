...
/// Runs a subprocess for generation steps that depend on external tools.
typedef PigeonProcessRunner =
...
class PigeonOptions {
  ...
  /// Path to the Dart FFI binding file that will be generated by ffigen.
  final String? dartFfiOut;
...
${_argParser.usage}''';
  ...
  static final ArgParser _argParser = ArgParser()
    ...
    ..addOption('dart_ffi_out', help: 'Path to Dart FFI binding file generated by ffigen (.dart).')
    ..addOption(
      'dart_ffi_config_out',
      help:
  ...
  static PigeonOptions parseArgs(List<String> args) {
    ...
    final dartFfiOut = results['dart_ffi_out'] as String?;
    final dartFfiBindingClass = results['dart_ffi_binding_class'] as String?;
    final dartFfiNativeLibrary = results['dart_ffi_native_library'] as String?;
    final hasDartFfiOptions =
        dartFfiBindingImport != null ||
        dartFfiOut != null ||
    ...
    final opts = PigeonOptions(
      ...
      dartOptions: hasDartFfiOptions
          ? DartOptions(
              ffiOptions: DartFfiOptions(
                bindingImportPath: dartFfiBindingImport,
                bindingClassName: dartFfiBindingClass ?? 'NativeLibrary',
                nativeLibraryExpression: dartFfiNativeLibrary ?? 'ffi.DynamicLibrary.process()',
              ),
  ...
  static Future<Error?> _runFfiGen(
    InternalPigeonOptions options,
    PigeonProcessRunner processRunner,
  ) async {
    final ffigenOptions = options.ffigenConfigOptions;
    if (ffigenOptions == null) {
      return null;
    }

    final String? workingDirectory = (options.basePath?.isEmpty ?? true) ? null : options.basePath;
    final ProcessResult result = await processRunner(Platform.resolvedExecutable, <String>[
      'run',
      'ffigen',
      '--config',
      ffigenOptions.configOut,
    ...
    final details = <String>[
      'ffigen failed with exit code ${result.exitCode}.',
      if (result.stderr.toString().trim().isNotEmpty) 'stderr:\n${result.stderr.toString().trim()}',
      if (result.stdout.toString().trim().isNotEmpty) 'stdout:\n${result.stdout.toString().trim()}',
    ];
  ...
  static Future<int> runWithOptions(
    ...
    @visibleForTesting PigeonProcessRunner? processRunner,
  }) async {
    ...
    var generatedFfiGenConfig = false;
    for (final adapter in safeGeneratorAdapters) {
      for (final FileType fileType in adapter.fileTypeList) {
        ...
        if (sink != null) {
          ...
          await sink.flush();
          await releaseSink(sink);
          if (adapter is FfiGenConfigGeneratorAdapter) {
            generatedFfiGenConfig = true;
          }
        }
      }
    }
